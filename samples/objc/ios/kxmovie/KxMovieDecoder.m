            ...
            error: (NSError **) perror
{
 ...
 fileStream = [[FileDataStream alloc] initWithFilePath:path];
  metaData = [[Metadata alloc] init];
  [metaData unserialize:fileStream];
  NSArray *segment = [metaData getStreams];
  fileSizeLeft = [[segment objectAtIndex:0]getInitSegmentLength];
  fileSize = [[segment objectAtIndex:0] getInitSegmentLength];
  
  fileData = [[NSData alloc] initWithBytes:[[segment objectAtIndex:0] getInitSegment] length:[[segment objectAtIndex:0] getInitSegmentLength]];
  
  unsigned char *miracle = malloc([[segment objectAtIndex:0] getInitSegmentLength]);
  memcpy(miracle, [[segment objectAtIndex:0] getInitSegment], [[segment objectAtIndex:0] getInitSegmentLength]);
  
  NSLog(@"data is %@ and miracle is %s ", fileData, miracle);
...
{
  /*NSData *buffer = [file readDataOfLength:buf_size];
  if ([buffer length] == 0)
    return 0;
  const unsigned char *bytePtr =
  (const unsigned char *)([buffer bytes]);
  memcpy(buf, bytePtr, buf_size);
  return buf_size; */
  if (cursor >= fileSize)
    return 0;
  if (cursor+buf_size > fileSize) {
    buf_size = fileSize - cursor;
  }
  uint8_t *byteData= (uint8_t*)malloc(buf_size);
  NSRange range = NSMakeRange(cursor, buf_size);
  NSLog(@"my range is %@", NSStringFromRange(range));
  
  [fileData getBytes:byteData range:range];
  const unsigned char *bytePtr =
...
{
  unsigned long long previous = [file offsetInFile];
  unsigned long long size = [file seekToEndOfFile];
  [file seekToFileOffset:previous];
  if (whence == AVSEEK_SIZE) {
    return size;
  }
  else if (size == -1 && whence == SEEK_END)
    return -1;
  if (whence == SEEK_CUR) {
...
- (kxMovieError) openInput: (NSString *) path
{
  ...
  bd.ptr  = buffer;
  bd.size = buffer_size;
  bd.filesize = buffer_size;
  avio_ctx_buffer = (uint8_t*)(av_malloc(avio_ctx_buffer_size));
  if (!avio_ctx_buffer)
    return kxMovieErrorOpenFile;
  avio_ctx = avio_alloc_context(avio_ctx_buffer, (int)avio_ctx_buffer_size, 0, &bd, &read_packet, NULL, &seek_packet);
  if (!avio_ctx)
...
- (KxAudioFrame *) handleAudioFrame
{
    ...
    vDSP_vflt16((SInt16 *)audioData, 1,
                (float*)(data.mutableBytes), 1, numElements);
    vDSP_vsmul((const float*)(data.mutableBytes), 1, &scale,
               (float*)(data.mutableBytes), 1,
               numElements);
